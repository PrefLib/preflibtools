...
def generate_mallows(
    ...
    if norm_phi:
        dispersions = [phi_from_normphi(num_alternatives, phi) for phi in dispersions]

    # Precompute the distros for each Phi.
    insert_distributions = []
    for i in range(len(mixture)):
        insert_distributions.append(
            mallows_insert_distributions(num_alternatives, dispersions[i])
        )
    ...
    for voter in range(num_voters):
        ...
        for i in range(1, len(insert_vector) + 1):
            # options are 1...max
            insert_vector[i - 1] = np.random.choice(
                range(1, i + 1), 1, p=insert_distributions[model][i]
            )[0]
...
def generate_mallows_mix(num_voters, alternatives, num_references, norm_phi=True):
    ...
    return generate_mallows(
        num_voters, len(alternatives), mixture, dispersions, references
    )
